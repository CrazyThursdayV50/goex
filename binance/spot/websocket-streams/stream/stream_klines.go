...
func (s klinesStreamer) StreamName() string {
	if s.timezone == "" {
		return spot.WsStream().Klines(strings.ToLower(s.symbol), s.interval)
	}

	return spot.WsStream().KlinesTimeZone(strings.ToLower(s.symbol), s.interval, s.timezone)
}
...
func (s *klinesStreamer) Connect(ctx context.Context) *wsclient.Client {
	client := newStream(
		s,
		s.logger,
