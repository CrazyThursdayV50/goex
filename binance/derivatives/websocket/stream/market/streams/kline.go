...
func (s *Stream) KlinesStream(streamName string, handler func(*klines.KlineData, error)) *Stream {
	stream := s.Clone()
...
func (s *Stream) ContinuousKlineStream(streamName string, handler func(*klines.KlineData, error)) *Stream {
	return s.KlinesStream(streamName, handler)
}

func (s *Stream) HandleKlineStreamEvent(f func(*klines.KlineData, error)) *Stream {
...
func (s *Stream) HandleContinuousKlineStreamEvent(f func(*klines.KlineData, error)) *Stream {
	s.RegisterEventHandler(
		continuous.Event,
		CreateBytesHandler(f),
	)
